...
public class LevelGenerator : MonoBehaviour
{
	...
	[HideInInspector]
	public SpaceShipMotor currentShipMotor;

	void Start ()
	{
		// Random init
		if (PlayerPrefs.GetInt ("GameTypeID") == MainMenu.GT_RANDOM) {
	...
	/*
		foreach (BackgroundObjects bgObject in levelProperties.bgObjects) {
			bgObject.normalSize.x = Random.Range (0.1f, 0.2f);
			bgObject.normalSize.z = bgObject.normalSize.x;
			bgObject.deltaSize.x = Random.Range (0.2f, 0.4f);
			bgObject.deltaSize.x = bgObject.deltaSize.z;
		}
		*/
