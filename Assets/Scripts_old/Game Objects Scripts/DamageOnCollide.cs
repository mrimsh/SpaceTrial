...
public class DamageOnCollide : MonoBehaviour
{
	...
	void OnCollisionStay (Collision collisionInfo)
	{
		if (Time.time > lastDamageTime + damageGainingFrequency) {
			foreach (ContactPoint contact in collisionInfo.contacts) {
				SpaceShipMotor_old targetSpaceShipMotor = contact.otherCollider.GetComponent<SpaceShipMotor_old> ();
				...
				if (globalProperties != null) {
					if (!globalProperties.invulnerable) {
						if (targetSpaceShipMotor != null) {
							targetSpaceShipMotor.DamageShip (targetDamage, (transform.parent.GetComponent<PlayerController_old> () == null) ? DamageSource.FriendlyShip : DamageSource.Player);
							GetCollisionDamage ();
						} else {
	...
	{
		GetComponent<SpaceShipMotor_old> ().DamageShip (selfDamage, DamageSource.Self);
